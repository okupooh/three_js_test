...
//	scene.background = new THREE.Color( 0xF0F0F0 );
	...
	//箱を作成
	const meshbox = getBoxMesh(width, height, depth);
	...
	_drills.forEach( drl => {
		const lathe = getDrillMeshCylinder(
			new THREE.Vector3( drl.x1 , drl.y1 , drl.z1 ) ,
			new THREE.Vector3( drl.x2 , drl.y2 , drl.z2 ) , drl.diameter , drl.tip ) ;
	
	//	scene.add(lathe);

	//	drillBSP = new ThreeBSP( lathe );
	//	boxBSP = new ThreeBSP( box );
	
	//	resultBSP = drillBSP.subtract( boxBSP ); 
	//	resultBSP = boxBSP.intersect( drillBSP );
	//	boxBSP = resultBSP ; 
		if ( unionBSP == null ) {
			unionBSP = new ThreeBSP( lathe );
//console.log( `first` );
		} else {
			drillBSP = new ThreeBSP( lathe );
			unionBSP = unionBSP.union( drillBSP ) ;
		//	unionBSP = resultBSP ;
//console.log( `union` );
		}
	});

	resultBSP = boxBSP.subtract( unionBSP );
	unionBSP = boxBSP.intersect( unionBSP );

	const unionMesh = unionBSP.toMesh();
/*
	unionMesh.updateMatrix();
	unionMesh.material.shading = THREE.FlatShading;
	unionMesh.material.opacity = 0.0 ;
	unionMesh.material.transparent = true ;
	unionMesh.material.wireframe  = true ;
	unionMesh.geometry.computeFaceNormals();
	unionMesh.geometry.computeVertexNormals();
	unionMesh.material.needsUpdate = true;
	unionMesh.geometry.buffersNeedUpdate = true;
	unionMesh.geometry.uvsNeedUpdate = true;
//	scene.add( unionMesh );
*/
...
//	resultMesh.updateMatrix();
	...
	const edges = new THREE.LineSegments(
		new THREE.EdgesGeometry( unionMesh.geometry , 1 ) ,
		new THREE.LineBasicMaterial( { color: 0xFFFFFF } ) 
	) ;
	unionMesh.add ( edges ) ;
	
	const boxedges = new THREE.LineSegments(
		new THREE.EdgesGeometry( box.geometry , 1 ) ,
		new THREE.LineBasicMaterial( { color: 0xFFFFFF } ) 
	) ;
	resultMesh.add ( boxedges ) ;
